	...
	return (
		<nav className="block sm:hidden">
			...
			<div className={`${isOpen ? 'block' : 'hidden'} overscroll-contain top-0 left-0 z-10 w-full fixed h-full md:block md:w-auto transform  translate-x-0 transition-transform ease-in-out duration-300 flex flex-row`} id="side-menu">
				<ul className="bg-black text-white w-4/5 h-full font-medium flex flex-col p-4 md:p-0  md:flex-row md:space-x-8 rtl:space-x-reverse md:mt-0 md:border-0 md:bg-white ">
					<SearchBar navClass="!block !p-0 !mt-6 !mb-6" btnClass="" />
				...
				<div onClick={() => handleOpenMenu(false)} className="w-1/5 h-full bg-red-500  opacity-70">
